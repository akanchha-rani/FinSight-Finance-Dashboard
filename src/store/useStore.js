...
function saveState(s) {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ txns: s.txns, role: s.role, dark: s.dark })) } catch {}
}
...
function setState(updater) {
  _state = { ...getState(), ...(typeof updater === 'function' ? updater(getState()) : updater) }
  saveState(_state)
...
export function useActions() {
  const addTxn = useCallback((t) => {
    setState(s => ({ txns: [t, ...s.txns].sort((a, b) => new Date(b.date) - new Date(a.date)) }))
  }, [])
