...
export function useAI(txns) {
  ...
  const expTxns = txns.filter(
    (t) => t.type === "expense" && new Date(t.date) >= cutoff30,
  );
  const expTxnsPrev = txns.filter(
    (t) =>
      t.type === "expense" &&
      new Date(t.date) >= cutoff60 &&
      new Date(t.date) < cutoff30,
  );
  const income30 = txns
    .filter((t) => t.type === "income" && new Date(t.date) >= cutoff30)
  ...
  const catDataPrevMap = Object.fromEntries(
    catDataPrev.map((c) => [c.category, c.amount]),
  );
  ...
  function buildContext() {
    const catSummary = catData
      .map((c) => `${c.category}: ${fmtCurrency(c.amount)} (${c.percentage}%)`)
      .join("\n");
    const prevSummary = catDataPrev
      .map((c) => `${c.category}: ${fmtCurrency(c.amount)}`)
      .join(", ");
    const recentTxns = txns
      ...
      .join("\n");
    const netTrend = monthlyData
      .map((m) => `${m.month}: ${fmtCurrency(m.net)}`)
      .join(", ");
  ...
  const fetchInsights = useCallback(async () => {
    ...
    try {
      const text = await callClaude(
        "You are a financial analyst. Return only valid JSON, no markdown fences.",
      ...
      setInsights([
        {
          ...
          body:
            savingsRate >= 20
              ? "Above the 20% benchmark — great discipline! Aim for 30% for faster wealth building."
              : "Below the 20% target. Cutting Food and Entertainment spending will help.",
          type: savingsRate >= 20 ? "success" : "warning",
          emoji: savingsRate >= 20 ? "🎯" : "⚠️",
        },
        {
          title: `Top Spend: ${catData[0]?.category || "N/A"}`,
          body: `${catData[0]?.category} consumed ${catData[0]?.percentage || 0}% of your budget (${fmtCurrency(catData[0]?.amount || 0)}). ${(catData[0]?.percentage || 0) > 30 ? "Consider a monthly cap." : "Looks manageable."}`,
          type: (catData[0]?.percentage || 0) > 30 ? "warning" : "info",
          emoji: "📊",
        },
        {
          title: `${expTxns.length} Expense Transactions`,
          body: `You made ${expTxns.length} expense transactions totalling ${fmtCurrency(exp30)} this month. Review regularly to spot leaks.`,
          type: "info",
          emoji: "📈",
        },
        {
          title: "Set Category Budgets",
          body: "A monthly cap on your top 3 categories reduces overspending significantly. Even rough limits work.",
          type: "info",
          emoji: "💡",
        },
        {
          title: "Automate Savings",
          body: "Auto-transfer at least 20% of salary to savings on payday. Spend what remains — not the other way around.",
          type: "success",
          emoji: "🏦",
        },
      ]);
